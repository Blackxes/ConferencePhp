...
class QueryParser {
	...
	public function template( Models\Query $query ) {
		...
		if ( !\Templax\Templax::$instance->has($query->get("key")) ) {
			...
			if ( is_null($query->get("postQuery")) )
				return new Models\Response( $query->get("rawRule"), "", $query );
			
			return new Models\Response( $query->get("rawRule"), null );
		}
	...
	public function templateSelect( Models\Query $query ) {
		...
		$content = \Templax\Templax::$instance->parse( (string) $query->get("value"), (array) $query->get("commandValue"), array(), $query->get("process") );
