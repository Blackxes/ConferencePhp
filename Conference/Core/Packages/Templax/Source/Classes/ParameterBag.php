...
class ParameterBag {
	...
	private function getParamInfo( $paramPath ) {
		...
		// 
		$info = array( "params" => &$this->params, "value" => null, "parent" => null, "paramKey" => null, "isBag" => false );
		...
		if ( method_exists($this, $paramKey) )
			$param = $this->{$paramKey}( $this->params[$paramKey] );

		// otherwise use the value when exists
		else if ( array_key_exists($paramKey, $this->params) )
			$param = &$this->params[ $paramKey ];
	...
	public function rMerge( $request, ...$params ) {
		...
		$info[ "params" ] = array_merge( call_user_func_array("array_merge", $params), $info["params"] );
