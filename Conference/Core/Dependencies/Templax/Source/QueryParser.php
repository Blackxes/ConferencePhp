...
class QueryParser {
	...
	public function case( Models\Query $query ) {
		...
		if ( $value == "" || is_array($value) && empty($value) || is_null($value) )
	...
	/**
	 * renders the inner context when the value is not defined
	 * @see ::case for the opposite case
	 * 
	 * @param \Templax\Source\Models\Query $query - the processing query
	 * 
	 * @return \Templax\Source\Models\Response
	 */
	public function caseNot( Models\Query $query ) {
	...
	public function foreach( Models\Query $query ) {
		...
		// row index / different from id since the id is defined by the user
		$index = 0;
		
		foreach( $query->get("value") as $id => $markup ) {

			// check for a hook
			$markupValue = \Templax\Templax::$instance->rParser->getHookValueFromCommandContext( $query, $id, (array) $markup );

			// add dynamic values
			$markupValue["tx-index"] = $index++;
			$markupValue["tx-item-id"] = $id;

			// provide a marker to print content without marker
			$markupValue["tx-item-content"] = is_scalar($markup) ? (string) $markup : "";
	...
	/**
	 * act the same as case but sets the scope to only the context of this command
	 * 
	 * @param \Templax\Source\Models\Query $query - the processing query
	 * 
	 * @return \Templax\Source\Models\Response
	 */
	public function frame( Models\Query $query ) {
	 ...
	 * @return \Templax\Source\Models\Response
	 */
	public function frameNot( Models\Query $query ) {
	...
	/**
	 * renders the inner context when the key is boolean true
	 * when null the key is not rendered and the else hidden as well
	 * 
	 * @param \Templax\Source\Models\Query $query - the processing query
	 * 
	 * @return \Templax\Source\Models\Response
	 */
	public function ifNot( Models\Query $query ) {
	...
	public function templateSelect( Models\Query $query ) {
		...
		$content = \Templax\Templax::$instance->parse( $query->get("value"), (array) $query->get("commandValue"), array(), $query->get("process") );
