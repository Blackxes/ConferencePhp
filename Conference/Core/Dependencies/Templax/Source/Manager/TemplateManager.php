...
class TemplateManager {
	...
	//
	public function registerTemplateInstance( Models\Template $template ) {
		...
		array $options = array())
	{
		...
		if ( !$id || !$filePath || $this->has($id) )
			return \Logfile\Logfile::log( "invalid values for template (id/config)", "error", 0, false );

		if ( $this->has($id) )
			return \Logfile\Logfile::log( "template {$id} already registered", "error", 0, false );
	...
	public function getFileTemplate( string $file, bool $cache = true ) {
		...
		if ( is_string($file) && $this->fileCache[$file] )
			...
			return \Logfile\Logfile::log( "Templax: file {$file} doesnt exist", "error", 404, null, "" );
	...
	//
	private function extractTemplate( string $file, string $marker ) {
